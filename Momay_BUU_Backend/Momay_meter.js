...
// ── ชื่อมิเตอร์ "จุดเดียว": ตั้ง env METER_KEY ตอน deploy (default 'deer' = ของเดิม) ──
// ทุก collection derive จากค่านี้ → เพิ่มมิเตอร์ใหม่ = แก้แค่ env ตัวเดียว ไม่ต้องแตะโค้ด
//   deer → pm_deers / *_deer   |   sand → pm_sands / *_sand   |   101 → pm_101s / *_101
const METER_KEY = (process.env.METER_KEY || 'deer').toLowerCase();
const COLL = {
  readings:  `pm_${METER_KEY}s`,
  peak:      `peak_notifications_${METER_KEY}`,
  dailyDiff: `daily_diff_notifications_${METER_KEY}`,
  test:      `test_notifications_${METER_KEY}`,
  dailyBill: `daily_bill_notifications_${METER_KEY}`,
  push:      `push_subscriptions_${METER_KEY}`,
};
const PM_deer = mongoose.model('PmReading', px_pm3250_schema, COLL.readings);
...
const PeakNotification = mongoose.model("PeakNotification", peakNotificationSchema, COLL.peak);
...
const DailyDiffNotification = mongoose.model("DailyDiffNotification", dailyDiffNotificationSchema, COLL.dailyDiff);
...
const TestNotification = mongoose.model("TestNotification", testNotificationSchema, COLL.test);
...
const DailyBillNotification = mongoose.model("DailyBillNotification", dailyBillNotificationSchema, COLL.dailyBill);
...
// Generic receiver — มิเตอร์ใหม่ POST/PUT เข้า /esp/<source> เช่น /esp/pm_101
// 1 backend = 1 มิเตอร์ → เก็บลง PM_deer ของ instance นี้เสมอ (source เป็นแค่ป้ายชื่อ)
// หมายเหตุ: /esp/pm_deer ถูกจับโดย route เฉพาะด้านบนก่อน (ลงทะเบียนก่อน)
async function _genericEspSave(req, res) {
  try {
    const source = (req.params.source || '').toLowerCase();
    if (!/^[a-z0-9_\-]+$/.test(source)) return res.status(400).json({ error: 'Invalid source' });
    const doc = await saveESPDoc(PM_deer, req.body || {});
    console.log(`💾 ${source} saved:`, doc._id);
    res.status(201).json({ success: true, id: doc._id });
  } catch (err) {
    console.error('❌ /esp/:source save error:', err);
    res.status(500).json({ error: err.message });
  }
}
app.post('/esp/:source', _genericEspSave);
app.put('/esp/:source', _genericEspSave);
...
app.get('/daily-energy', async (req, res) => {
  try {
    ...
    res.json({
      date: queryDate,
      series: [
        { label: `pm_${METER_KEY}`, points: pointsTotal },
...
app.get('/daily-energy/:source', async (req, res) => {
  try {
    ...
    // 1 backend = 1 มิเตอร์ → เสิร์ฟ PM_deer ของ instance นี้ไม่ว่า source จะชื่ออะไร
    // (source ใช้แค่ระบุมิเตอร์ฝั่ง frontend; แต่ละมิเตอร์ deploy เป็น instance แยก)
    const Model = PM_deer;
...
const PushSubscription = mongoose.model('PushSubscription', pushSubscriptionSchema, COLL.push);
...
async function sendPushNotification(title, body, type = 'test', data = {}) {
  try {
    ...
    // 1. บันทึกลง Database แยกตาม type — ใช้โมเดลระดับบนสุด (collection *_deer)
...
app.get('/api/notifications/all', async (req, res) => {
  try {
    ...
    // Use top-level models (collections *_deer)
